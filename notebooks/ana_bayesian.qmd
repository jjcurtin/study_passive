...
auroc_avg <- read_csv(here::here(path_models_passive, 
...
pp <- pass |> 
  ...
  perf_mod(formula = statistic ~ model + (1 | id),
         ...
         iter = 6000, chains = 4, adapt_delta = .9, # increased iteration from 2000 to fix divergence issues
         family = gaussian, 
)  
...
contrasts <- pp |>
  ...
  mutate(contrast = factor(contrast,
                           levels = c("all vs pass", "all vs act"),
                           labels = c("Both vs. Passive Context", "Both vs. Active Context")))
...
```{r}
# save data for use in mak_figures and mak_tables
pp_tidy |>
  write_csv(here::here(path_models_passive, "posteriors.csv"))

pp_perf_tibble |>
  write_csv(here::here(path_models_passive, "pp_perf_tibble.csv"))

contrasts |>
  write_csv(here::here(path_models_passive, "contrasts.csv"))
```